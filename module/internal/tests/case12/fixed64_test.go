	...
	base.FrameTest(t, "msg.pb.bin", func(t *testing.T, fs afero.Fs, res *bytes.Buffer) {
		...
		assert.Equal(t,
			"github.com/yimi-go/protoc-gen-validate-jsonschema"+
				"/module/internal/tests/case12/msg.pb.Fixed64TestMsg.schema.json",
			file.GetName())
		t.Logf("\n%s", file.GetContent())
		type schema struct {
			Type             any                `json:"type"`
		...
		assert.Equal(t, "#/definitions/case12.Fixed64TestMsg", sf.Ref)
		assert.NotEmpty(t, sf.Definitions)
		ms, ok := sf.Definitions["case12.Fixed64TestMsg"]
		if !ok {
...

		{
			...
			assert.Equal(t, "uint64", prop.Format)
			assert.NotNil(t, prop.Minimum)
			assert.GreaterOrEqual(t, *prop.Minimum, uint64(0))
			assert.False(t, prop.ExclusiveMinimum)
			assert.NotNil(t, prop.Default)
			assert.Equal(t, uint64(0), *prop.Default)
...

		{
			...
			assert.Equal(t, uint64(1), *prop.XConst)
		}

		{
			...
			assert.Equal(t, uint64(10), *prop.Maximum)
...

		{
			...
			assert.Equal(t, uint64(0), *prop.Default)
			assert.NotNil(t, prop.Maximum)
			assert.Equal(t, uint64(9), *prop.Maximum)
			assert.True(t, prop.ExclusiveMaximum)
		}
...

		{
			...
			assert.Equal(t, uint64(0), *prop.Default)
			assert.NotNil(t, prop.Minimum)
			assert.Equal(t, uint64(10), *prop.Minimum)
...

		{
			...
			assert.Equal(t, uint64(0), *prop.Default)
			assert.NotNil(t, prop.Minimum)
			assert.Equal(t, uint64(11), *prop.Minimum)
			assert.True(t, prop.ExclusiveMinimum)
		}

		{
			...
			assert.NotNil(t, prop.Default)
			assert.Equal(t, uint64(0), *prop.Default)
			assert.Equal(t, []uint64{1, 2, 3, 4, 5}, prop.Enum)
		}

		{
			...
			assert.Equal(t, []uint64{7, 8, 9}, prop.Not.Enum)
		}
	})
}
