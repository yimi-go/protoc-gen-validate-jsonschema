...
func TestUInt32Wrapper(t *testing.T) {
	base.FrameTest(t, "msg.pb.bin", func(t *testing.T, fs afero.Fs, res *bytes.Buffer) {
		...
		assert.Equal(t,
			"github.com/yimi-go/protoc-gen-validate-jsonschema"+
				"/module/internal/tests/case26/msg.pb.UInt32WrapperTestMsg.schema.json",
			file.GetName())
		t.Logf("\n%s", file.GetContent())
		type schema struct {
		...
		assert.Equal(t, "#/definitions/case26.UInt32WrapperTestMsg", sf.Ref)
		assert.NotEmpty(t, sf.Definitions)
		ms, ok := sf.Definitions["case26.UInt32WrapperTestMsg"]
		if !ok {
...

		{
			...
			assert.GreaterOrEqual(t, *prop.Minimum, uint32(0))
...

		{
			...
			assert.Equal(t, uint32(1), *prop.XConst)
		}

		{
			...
			assert.Equal(t, uint32(10), *prop.Maximum)
...

		{
			...
			assert.Nil(t, prop.Default)
			assert.NotNil(t, prop.Maximum)
			assert.Equal(t, uint32(9), *prop.Maximum)
			assert.True(t, prop.ExclusiveMaximum)
		}
...

		{
			...
			assert.Nil(t, prop.Default)
			assert.NotNil(t, prop.Minimum)
			assert.Equal(t, uint32(10), *prop.Minimum)
...

		{
			...
			assert.Nil(t, prop.Default)
			assert.NotNil(t, prop.Minimum)
			assert.Equal(t, uint32(11), *prop.Minimum)
			assert.True(t, prop.ExclusiveMinimum)
		}

		{
			...
			assert.False(t, prop.ExclusiveMinimum)
			assert.Nil(t, prop.Default)
			assert.Equal(t, []uint32{1, 2, 3, 4, 5}, prop.Enum)
		}

		{
			...
			assert.Equal(t, []uint32{7, 8, 9}, prop.Not.Enum)
		}
	})
}
