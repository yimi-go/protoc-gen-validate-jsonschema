	...
	base.FrameTest(t, "msg.pb.bin", func(t *testing.T, fs afero.Fs, res *bytes.Buffer) {
		...
		assert.Equal(t,
			"github.com/yimi-go/protoc-gen-validate-jsonschema"+
				"/module/internal/tests/case11/msg.pb.Fixed32TestMsg.schema.json",
			file.GetName())
		t.Logf("\n%s", file.GetContent())
		type schema struct {
		...
		assert.Equal(t, "#/definitions/case11.Fixed32TestMsg", sf.Ref)
		assert.NotEmpty(t, sf.Definitions)
		ms, ok := sf.Definitions["case11.Fixed32TestMsg"]
		if !ok {
...

		{
			...
			assert.Equal(t, uint32(0), *prop.Default)
